/* SC-191 ROUND 3 — the `roster` base, decided one AXIS at a time.
 *
 * WHAT CHANGED SINCE ROUND 2. Scott ruled `roster` the base to iterate ("a lot going
 * for it and I think is likely a solid base"), named three defects, and asked to judge
 * them separately: "I think I want to see some variants of all the things ive brought
 * up here so I can decide." So round 3 is NOT a blended proposal. It is one board with
 * three independent attributes on the card root —
 *
 *   data-crest = before | dark | none | rule   does the hero monogram earn its place?
 *   data-seal  = before | ink | struck | bare  the "super bright" test-result circles
 *   data-space = before | pad  | centre        the refinement pass
 *
 * — each of which can be answered without answering the others. `before` on every axis
 * is the round-2 roster verbatim, so a shot named `-before-` is the honest control and
 * not a strawman.
 *
 * THE ONE UNIVERSAL CHANGE, present in every shot including `before`: the "add a hero"
 * ROW is deleted and the affordance is a small + button in the top-left "Heroes" header
 * cell. Scott: "that button should be moved to the top-left 'Heroes' header cell as a
 * little 'plus' button or something." It is not an axis, so it is not held constant —
 * it is simply true everywhere.
 *
 * THIS FILE IS A COPY OF mock2.js, ON PURPOSE. mock2.js + round2.css are the frozen
 * record of what Scott actually reviewed in round 2; editing them would rewrite the
 * "before". round3.css layers ONLY the axis overrides on top of round2.css's roster
 * skin, so `before` is produced by the round-2 code path itself.
 *
...
(function () {
	...
	function buildCard(root, m, d, opts) {
		...
		// `data-treat="roster"` is fixed: round 3 iterates Scott's chosen base, it does not
		// re-litigate it. The three axis attributes are what round3.css reads.
		attr(c, {
			'data-treat': 'roster',
	...
	/* THE AXIS RESOLVER.
	 *
	 * `?axis=<name>&option=<value>` names the ONE axis under review; the other two are
	 * pinned to the round-3 RECOMMENDATION so each option is seen as it would actually
	 * ship rather than beside the two defects it does not fix. The exception is
	 * `option=before`, which pins ALL THREE to `before` — that is the single honest
	 * control, identical whichever axis names it. `axis=all&option=rec` is the assembled
	 * recommendation.
	 *
	 * If Scott mixes picks across axes (say crest=dark with seal=struck) the combination
	 * will not have been photographed; that is inherent to deciding axis-by-axis and is
	 * the trade he asked for.
	 */
	const RECOMMEND = { crest: 'none', seal: 'ink', space: 'centre' };
	const BEFORE = { crest: 'before', seal: 'before', space: 'before' };
	function resolveAxes(axis, option) {
		if (option === 'before') return Object.assign({}, BEFORE);
		if (axis === 'all') return Object.assign({}, RECOMMEND);
		const out = Object.assign({}, RECOMMEND);
		if (!(axis in out)) throw new Error('unknown axis: ' + axis);
		out[axis] = option;
		return out;
	}
	...
	function boot() {
		...
		const axis = (q.get('axis') || 'all').toLowerCase();
		const option = (q.get('option') || 'rec').toLowerCase();
		const axes = resolveAxes(axis, option);
		...
		window.__sc191r3Done = { axis: axis, option: option, axes: axes, state: stateKey, bg: bg };
	}
