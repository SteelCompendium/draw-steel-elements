...
// SC-202 r6b fix round (independent review MED-1) — this sweep's own navigations used to
// build their query without `sheet: '1'`, which the first r6b commit defended as "not
// tautological, so no reformulation needed". True, but beside the point the reviewer
// raised: EVERY real screen capture now renders under the real sheet + the real
// `.markdown-preview-view` wrapper + accepted-truth `box-sizing: border-box`, and this
// gate was still measuring the OLD unwrapped, `content-box`, no-host-CSS-at-all page — a
// box model nothing ships in any more. Fix: navigate WITH `sheet: '1'` (so `#mount` is
// wrapped and `box-sizing: border-box` applies, exactly like every other capture) and
// then explicitly disable the link before the bare sample — the same "disable-for-bare"
// step every other sweep in this file takes — so the bare pass still has NO Obsidian CSS
// at all (preserving this sweep's own can-fail power) while the DOM/box model it measures
// finally matches reality. The `injectHostCss`/`OBSIDIAN_HOST_BUTTON_CSS` host mechanism
// itself is UNCHANGED — that hand-modeled overlay is still what "host" means here, still
// independent of the real sheet, still drift-pinned by `assertHostCopyPinnedToObsidian`
// against the live installed Obsidian.
...
// SC-202 r6b fix round (independent review MED-1) — this sweep used to navigate without
// `sheet: '1'`, defended in the first r6b commit as "never made tautological, so nothing
// to reformulate". True but beside the point: every real screen capture now renders
// buttons under the real sheet, the real `.markdown-preview-view` wrapper and
// accepted-truth `box-sizing: border-box`, and this gate was still measuring the OLD
// unwrapped `content-box` page — proving nothing about the box model any capture actually
// ships. Fix, same shape as `assertChromeHostLeak`'s own: navigate WITH `sheet: '1'`, then
// explicitly disable the link before the bare sample (the "disable-for-bare" step every
// other sweep in this file takes) — bare stays genuinely host-CSS-free (preserving this
// sweep's own can-fail power, proven below), while the DOM/box model finally matches
// reality. `injectHostCss`/`OBSIDIAN_HOST_BUTTON_CSS` (the hand-modeled, drift-pinned
// subject SC-203/SC-205 own) is UNCHANGED as the "host" mechanism — unrelated to
// `dist/obsidian-app.css`, still cross-checked against the live installed Obsidian by
// `assertHostCopyPinnedToObsidian`. Can-fail re-proven under this formulation:
// `sc202-r6bfix-canfail-button.log`.
 ...
 * see `applyRealObsidianCascade` in entry.ts), every host-leak sweep in this file
 * (input/table/list/inline/checkbox/link-token-override, and — fix round MED-1 — button
 * and chrome too) navigates WITH `sheet: '1'` (so the page's real cascade — link enabled,
 * `#mount` wrapped in the real `.markdown-preview-view.markdown-rendered` ancestor — is
 * exactly what a real capture gets) and then toggles the SAME link's `.disabled` for its
 * own bare-vs-host pass, rather than injecting (and never removing) a second copy of the
 * same stylesheet. This is what keeps every sweep non-tautological now that the sheet
 * defaults to on for screen combos: bare is the link EXPLICITLY disabled, host is it
 * EXPLICITLY re-enabled — the invariance under test ("plugin computed style is identical
 * with and without Obsidian's sheet") is unchanged, only the toggle mechanism is.
 *
 * FIX ROUND (independent review MED-2) — a toggle that silently did nothing used to be
 * indistinguishable from a real one: emptying `dist/obsidian-app.css` in an isolated copy
 * left all six original sweeps green with byte-identical comparison counts (the reviewer's
 * `sc202-r6brev-v1-shots.log`), and `link.disabled = true` DETACHES the sheet
 * (`link.sheet === null`) while re-enabling it does NOT reattach within the same
 * `page.evaluate` call — only across a genuine round-trip (reproduced: same-call
 * re-enable-then-read, even with a forced reflow + an awaited `requestAnimationFrame`,
 * still reads the STALE value; a second, separate `page.evaluate` reads the correct one
 * — usually. A FURTHER race showed up proving this fix: re-ENABLING a 637 KB `file://`
 * stylesheet is a real, asynchronous fetch+parse, not a synchronous re-attach — debugged
 * live (`el.disabled === false` but `el.sheet === null`, i.e. still loading) on both a
 * heavy `gallery=1` page and a plain single-fixture one, so it is genuine load latency,
 * not a page-weight effect. So this function now (1) toggles `disabled` in one round-trip,
 * (2) when enabling, explicitly WAITS (`page.waitForFunction`, real timeout) for
 * `link.sheet !== null` — i.e. for the browser to confirm the stylesheet actually
 * attached, not merely that the attribute was set — and only THEN (3), in a separate
 * round-trip, reads a SENTINEL token only the real sheet can produce (`--list-indent`,
 * real app.css: `calc(0.5625em * 4)`; this file's own vars.css fallback: the literal
 * `2em` — the two can never coincide) and throws loudly if the resolved value still does
 * not match what was requested. (`link.sheet.cssRules.length` was considered as a second,
 * independent sentinel — SC-205's own `assertHostCopyPinnedToObsidian` pattern — but
 * `file://` pages throw a `SecurityError` reading `cssRules` cross-stylesheet in this
 * harness's sandbox, so the computed-style sentinel is the only reliable signal
 * available here.) This makes the "emptied sheet" failure mode loud: every sweep's very
 * first `setHostSheetEnabled(page, true)` call now throws instead of silently sampling
 * vars.css's fallback and calling it "the real sheet".
 */
async function setHostSheetEnabled(page, enabled) {
	...
	if (enabled) {
		// Wait for the browser to actually finish attaching the stylesheet (a real fetch +
		// parse of a 637 KB file, not a synchronous side effect of clearing `.disabled`) —
		// see the comment above for the live debug that found this. 10 s is generous; a
		// genuinely broken/missing sheet fails the sentinel check below immediately once
		// this resolves, it never silently times out into a false pass.
		await page
			.waitForFunction(() => document.getElementById('dse-obsidian-app-css')?.sheet != null, null, {
				timeout: 10000,
			})
			.catch(() => {
				/* fall through to the sentinel check below, which reports the real failure */
			});
	}
	// A SEPARATE evaluate call — see the comment above for why the toggle and the read must
	// each be a genuine round-trip, never combined into one.
	const sentinel = await page.evaluate(() =>
		getComputedStyle(document.body).getPropertyValue('--list-indent').trim(),
	);
	const REAL_SHEET_VALUE = 'calc(0.5625em * 4)';
	if (enabled && sentinel !== REAL_SHEET_VALUE) {
		throw new Error(
			`OBSIDIAN APP.CSS NOT APPLIED — setHostSheetEnabled(page, true) but --list-indent ` +
				`reads "${sentinel}", expected the real sheet's own "${REAL_SHEET_VALUE}". The ` +
				`injected stylesheet is not live (empty/corrupt dist/obsidian-app.css, or it never ` +
				`finished attaching).\n${HASH_FAILURE_REMEDY}`,
		);
	}
	if (!enabled && sentinel === REAL_SHEET_VALUE) {
		throw new Error(
			`OBSIDIAN APP.CSS STILL APPLIED — setHostSheetEnabled(page, false) but --list-indent ` +
				`still reads the real sheet's own "${REAL_SHEET_VALUE}"; the "bare" sample below ` +
				`would not be bare.\n${HASH_FAILURE_REMEDY}`,
		);
	}
}
 ...
 *  vars.css's own comment.
 *
 *  FIX ROUND (independent review LOW-1) — the first r6b commit left `boxSizing`/
 *  `overflowWrap` IN this relative list, printing a gate line that implied they were
 *  compared. They cannot be: the wrapper rule above is untoggled by design, so bare and
 *  host read the identical value BY CONSTRUCTION — the relative comparison can never fail,
 *  which is exactly the un-failable-gate shape the ticket exists to close. Moved OUT of
 *  this list into `assertTableCellBoxModelIsAccepted`'s own ABSOLUTE assertion below (run
 *  once per scheme, under the sheet, against a real sampled cell) — pinning the accepted
 *  truth as a real, can-fail check instead of a relative comparison with nothing to
 *  compare against. */
...
async function assertTableHostLeak(page) {
	...
	for (const bg of ['dark', 'light']) {
		...
		// FIX ROUND (independent review LOW-1) — box-sizing/overflow-wrap as an ABSOLUTE
		// assertion, not a relative bare-vs-host comparison: `vars.css`'s wrapper rule is
		// untoggled by design (§ this file's own header), so a relative comparison here can
		// NEVER fail — it would print a gate line implying coverage it does not have. Read
		// separately (a dedicated call — `TABLE_CELL_PROPS` no longer carries either
		// property) on every real cell already tagged for the host pass, against the literal
		// accepted value, so a regression that stops the wrapper rule from reaching a real
		// table cell is still caught.
		const hostBoxModelCells = await page.evaluate(readTaggedCells, ['boxSizing', 'overflowWrap']);
		for (const c of hostBoxModelCells) {
			boxModelChecked += 1;
			if (c.boxSizing !== 'border-box') {
				problems.push(
					`${bg}|cell|${c.key}(${c.tag}) ABSOLUTE: boxSizing is "${c.boxSizing}", expected ` +
						`"border-box" (accepted Obsidian truth, vars.css)`,
				);
			}
			if (c.overflowWrap !== 'break-word') {
				problems.push(
					`${bg}|cell|${c.key}(${c.tag}) ABSOLUTE: overflowWrap is "${c.overflowWrap}", expected ` +
						`"break-word" (accepted Obsidian truth, vars.css)`,
				);
			}
		}
	...
	console.log(
		`\ntable host-leak OK (${kindCount} table kinds × dark/light = ${comparisons} comparisons ` +
			...
			`ancestor: every sampled table/row/cell property is identical with and without it; ` +
			`${TABLE_CELL_PROPS_EXCLUDED.join(' and ')} excluded by design; ` +
			`${boxModelChecked} cells' box-sizing/overflow-wrap ABSOLUTE-asserted border-box/` +
			`break-word — SC-202 r6b fix round LOW-1, accepted Obsidian truth pinned, not compared)`,
	);
}
...
// still true here: once app.css is injected the pointer-centre math goes stale); (b) a
// genuine NUMERIC-SPECIFICITY TIE risk (`.markdown-rendered .internal-link` is EXACTLY
// this block's own anchor specificity) that a plain bare-vs-host diff cannot distinguish
// from a coincidence — closed with a dedicated `body`-scoped token-override probe
// (`assertLinkTokenOverride` below), the same method round 2's MED-1 review introduced.
const HEADING_PROPS = [
	'fontSize',
	'fontWeight',
	'lineHeight',
...
/** Tag every REAL h1-h6/strong/em/mark/code/a the gallery (or a named-fixture visit)
 *  renders under a `[data-dse-element]`/`.dse-modal` root — same running-index-per-node
 *  convention `tagLists` uses (not a de-duped "kinds" sample: two structurally-identical
 *  headings are compared independently). Returns per-kind counts so the caller can
 *  hard-fail if a family this round claims to cover renders 0 real nodes.
 *
 *  The selector is built INSIDE the function, not closed over from a module-level const:
 *  `page.evaluate` serializes only this function's own source text (the same reason every
 *  other reader in this file takes its prop list as an ARGUMENT rather than closing over
...
function tagInline() {
	...
	for (const n of document.querySelectorAll(selector)) {
		...
		const key = (root ? root.getAttribute('data-dse-element') : '(modal)') + '|' + kind + '#' + i;
		n.setAttribute('data-dse-inlineleak', key);
		n.setAttribute('data-dse-inlineleak-kind', kind);
		i += 1;
		counts[kind] += 1;
	}
	return counts;
...
/** Read every tagged node's rest-state props (per-kind, via `propsByKind`) plus, for a
 *  `.external-link`-classed anchor, its own extra icon-material props (hardcoded literally
...
/** FIX ROUND-shaped synthetic probe (same method as r3's `withSyntheticLiP`/
 *  `readNestedListMargin`): `h1`/`h5`/`mark`/`code` have NO live tag anywhere in the
 *  corpus (censused — h2/h3/h4/h6 all occur for real, see the styles-source.css block's
 *  own "THE PLUGIN'S OWN CSS" note), so this builds one of each under the first tagged
 *  card body, measures, and removes — never left in the DOM, never screenshotted. Returns
 *  `null` if no tagged host exists to attach to. `propsByKind` is a PARAMETER, same
 *  serialization reason as `readTaggedInline`. */
function withSyntheticInline(propsByKind) {
	const host =
		document.querySelector('[data-dse-element] .dse-card__body') ||
		document.querySelector('[data-dse-element] .dse-md-inline') ||
		document.querySelector('[data-dse-element] .dse-feature, [data-dse-element] .dse-card');
	if (!host) return null;
	const kinds = ['h1', 'h5', 'mark', 'code'];
	const rec = {};
	for (const kind of kinds) {
		...
		const cs = getComputedStyle(el);
		const r = {};
		for (const p of propsByKind[kind]) r[p] = cs[p];
		rec[kind] = r;
		el.remove();
	}
	return rec;
...
/** Focus ONE tagged link and report whether it reached `:focus-visible` — same shape as
 *  `focusInputTagged`, minus the disabled/box-less exemptions no `<a>` in this gallery
 *  ever needs (an anchor is never `disabled`, and every tagged one renders a real box). */
function focusLinkTagged(key) {
	const n = document.querySelector(`[data-dse-inlineleak="${key}"]`);
	if (!n) return null;
	n.scrollIntoView({ block: 'center', inline: 'center' });
	n.focus();
	return { active: n.matches(':focus-visible') };
}

/** Read one tagged link's `LINK_REST_PROPS`/`LINK_HOVER_PROPS` (both hardcoded literally,
 *  same serialization reason as `readTaggedInline`) plus whether it currently matches the
 *  given pseudo-class — same "never absorbed" contract as `readOneInputTaggedMatching`.
 *  `{ key, pseudo }` is a single PARAMETER object (page.evaluate accepts exactly one
 *  argument to the page function — same shape `readTaggedLists`'s own `{ attr, props }`
 *  uses), not two, and not a wrapper arrow function either: an arrow function passed to
 *  `page.evaluate` is serialized on its OWN, and a call inside it to a same-file sibling
 *  function (`readOneLinkTagged`) is just as much a free reference across the Node/page
...
function readOneLinkTagged({ key, pseudo }) {
	...
	if (!n) return null;
	const cs = getComputedStyle(n);
	const rec = { key, active: pseudo ? n.matches(pseudo) : true };
	for (const p of linkRestProps) rec[p] = cs[p];
	for (const p of linkHoverProps) rec['hover_' + p] = cs[p];
	return rec;
}

/** CDP `CSS.forcePseudoState(['hover'])` pass over every tagged link — same method
 *  `probeTableRowsInState`/`assertInputHostLeak`'s own hover branch use (Playwright's own
 *  `hover()`/`mouse.move()` stop engaging once app.css is injected — INFO-3). */
async function probeLinksHover(page, cdp, docRootNodeId, linkKeys) {
	const records = [];
	const problems = [];
	for (const key of linkKeys) {
		const found = await cdp.send('DOM.querySelector', { nodeId: docRootNodeId, selector: `[data-dse-inlineleak="${key}"]` });
		if (!found?.nodeId) {
			problems.push(`${key}: CDP could not resolve the tagged node`);
			continue;
		}
		await cdp.send('CSS.forcePseudoState', { nodeId: found.nodeId, forcedPseudoClasses: ['hover'] });
		const rec = await page.evaluate(readOneLinkTagged, { key, pseudo: ':hover' });
		await cdp.send('CSS.forcePseudoState', { nodeId: found.nodeId, forcedPseudoClasses: [] });
		if (!rec) {
			problems.push(`${key}: the tagged node vanished mid-sweep`);
			continue;
		}
		if (!rec.active) {
			problems.push(`${key}: CSS.forcePseudoState(['hover']) did not make the node match :hover`);
			continue;
		}
		records.push(rec);
	}
...
/** `:focus-visible` pass — real `.focus()` after establishing keyboard modality once per
 *  pass (`page.mouse.move(0,0)` + Tab), same shape `assertInputHostLeak`'s own focus-
 *  visible branch uses. Every tagged `<a>` in this gallery renders a real, focusable box
 *  (never `disabled`), so unlike the input/button sweeps this has no exemption ladder. */
async function probeLinksFocusVisible(page, linkKeys) {
	await page.mouse.move(0, 0);
	await page.keyboard.press('Tab');
	const records = [];
	const problems = [];
	for (const key of linkKeys) {
		const res = await page.evaluate(focusLinkTagged, key);
		if (!res) {
			problems.push(`${key}: the tagged node vanished mid-sweep`);
			continue;
		}
		const rec = await page.evaluate(readOneLinkTagged, { key, pseudo: ':focus-visible' });
		if (!rec) {
			problems.push(`${key}: the tagged node vanished mid-sweep`);
			continue;
		}
		if (!rec.active) {
			problems.push(`${key}: .focus() did not make the node match :focus-visible`);
			continue;
		}
		records.push(rec);
	}
...
// Gallery mounts each element's DEFAULT fixture only (same "must also visit by its own
...
async function assertInlineHostLeak(page) {
	...
	for (const bg of ['dark', 'light']) {
		for (const visit of INLINE_SWEEP_VISITS) {
			...
			if (blind.length) {
				problems.push(`${bg}|${visit.label}: ${blind.map(([k, m]) => `only ${counts[k] ?? 0} ${k} (need ${m})`).join(', ')} — the sweep is blind`);
				continue;
			}
			for (const k of Object.keys(kindCounts)) if (k !== 'external') kindCounts[k] = Math.max(kindCounts[k], counts[k] ?? 0);

			const bareRest = await page.evaluate(readTaggedInline, INLINE_PROPS_BY_KIND);
			const linkKeys = bareRest.filter((r) => r.kind === 'a').map((r) => r.key);
			const { root: bareRoot } = await cdp.send('DOM.getDocument', { depth: -1 });
			const bareHover = await probeLinksHover(page, cdp, bareRoot.nodeId, linkKeys);
			for (const p of bareHover.problems) problems.push(`${bg}|${visit.label}|host-absent|hover: ${p}`);
			const bareFocus = await probeLinksFocusVisible(page, linkKeys);
			for (const p of bareFocus.problems) problems.push(`${bg}|${visit.label}|host-absent|focus-visible: ${p}`);
			...
			const hostRestByKey = new Map(hostRest.map((r) => [r.key, r]));
			for (const b of bareRest) {
				const h = hostRestByKey.get(b.key);
				if (!h) {
					problems.push(`${bg}|${visit.label}|${b.kind}|${b.key}: vanished when the host sheet was added`);
					continue;
				}
				restComparisons += 1;
				for (const p of INLINE_PROPS_BY_KIND[b.kind] ?? []) {
					if (b[p] !== h[p]) problems.push(`${bg}|${visit.label}|${b.kind}|${b.key}: Obsidian's real app.css changes ${p} — "${b[p]}" without the host, "${h[p]}" with it`);
				}
				if (b.isExternal) {
					externalComparisons += 1;
			...
			const hostHover = await probeLinksHover(page, cdp, hostRoot.nodeId, linkKeys);
			for (const p of hostHover.problems) problems.push(`${bg}|${visit.label}|host-present|hover: ${p}`);
			const hostHoverByKey = new Map(hostHover.records.map((r) => [r.key, r]));
			for (const b of bareHover.records) {
				const h = hostHoverByKey.get(b.key);
				if (!h) {
					problems.push(`${bg}|${visit.label}|a:hover|${b.key}: vanished when the host sheet was added`);
					continue;
				}
				hoverComparisons += 1;
			...
			const hostFocus = await probeLinksFocusVisible(page, linkKeys);
			for (const p of hostFocus.problems) problems.push(`${bg}|${visit.label}|host-present|focus-visible: ${p}`);
			const hostFocusByKey = new Map(hostFocus.records.map((r) => [r.key, r]));
			for (const b of bareFocus.records) {
				const h = hostFocusByKey.get(b.key);
				if (!h) {
					problems.push(`${bg}|${visit.label}|a:focus-visible|${b.key}: vanished when the host sheet was added`);
					continue;
				}
				focusComparisons += 1;
			...
			if (visit.synth && bareSynth) {
				const hostSynth = await page.evaluate(withSyntheticInline, INLINE_PROPS_BY_KIND);
				if (hostSynth) {
					for (const kind of Object.keys(bareSynth)) {
						synthComparisons += 1;
						for (const p of INLINE_PROPS_BY_KIND[kind]) {
							if (bareSynth[kind][p] !== hostSynth[kind][p]) {
								problems.push(
									`${bg}|${visit.label}|synthetic|${kind}: Obsidian's real app.css changes ${p} — "${bareSynth[kind][p]}" without the host, "${hostSynth[kind][p]}" with it`,
								);
							}
						}
					}
				} else {
					problems.push(`${bg}|${visit.label}|synthetic probe: no tagged card body to attach to (host pass)`);
				}
			}
		}
	}
	if (problems.length) {
		const shown = problems.slice(0, 60);
		console.error(
			`\nINLINE HOST-LEAK VIOLATED — with the real Obsidian app.css present (and the ` +
				`.markdown-preview-view.markdown-rendered ancestor a real vault always supplies) the ` +
				`plugin's own headings/emphasis/links do not hold their own type or material:\n` +
				shown.map((p) => `  ${p}`).join('\n') +
				(problems.length > shown.length ? `\n  … and ${problems.length - shown.length} more` : '') +
				`\nSee styles-source.css → "SC-202 r4 — HEADING + EMPHASIS + LINK HOST RE-GROUNDING".`,
		);
		process.exit(1);
	}
...
/** SC-202 r4 — round-2 MED-1's own method applied to this round's one real tie risk: does
 *  the `.internal-link` companion (styles-source.css GROUP 6) win because it is
 *  STRUCTURALLY ahead of Obsidian's own `.markdown-rendered .internal-link` rule, or only
 *  because the two currently happen to compute the same values? Overriding the tokens
 *  Obsidian's rule actually reads (`--link-color`/`--link-weight`/`--link-decoration`/
 *  `--link-decoration-thickness`/`--cursor-link`) at `body` scope to deliberately absurd
 *  values and re-measuring proves it: this block's own companion rule never reads any of
 *  these custom properties (it hardcodes the browser's literal UA defaults instead), so a
 *  real structural win reports 0 diffs regardless of what the tokens are; a coincidental
 *  win would leak the instant a real vault's theme picked different token values. */
...
try {
	...
	if (!args.element) {
		...
		// SC-202 r4 — the same question asked of every HEADING/EMPHASIS/LINK the plugin
