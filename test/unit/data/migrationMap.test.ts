...
import {
	MIGRATION_MAP, MIGRATION_MAP_SCHEMA_VERSION, validateMigrationMap,
...
describe("the shipped migration map", () => {
	...
	test("every hashed (final-release) entry has a distinct destination", () => {
		...
		for (const [newPath, hash] of Object.values(MIGRATION_MAP.paths)) {
	...
	test("the recorded counts agree with the map itself", () => {
		...
		const hashed = Object.values(MIGRATION_MAP.paths).filter((entry) => entry.length === 2).length;
