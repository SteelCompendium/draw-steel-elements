...
export class GenericElementProcessor {
    plugin: Plugin;
    view: any;
    model: any;
    element_name: string;
    ...
    constructor(plugin: Plugin, view: any, model: any, element_name: string, edit_mode_safe: boolean = false) {
        this.plugin = plugin;
        this.view = view;
        this.model = model;
    ...
    public postProcess(source: string, el: HTMLElement, ctx: MarkdownPostProcessorContext): void | Promise<any> {
        genericElementPostProcess(this.plugin, source, el, ctx, this.view, this.model, this.element_name, this.edit_mode_safe);
    }
}

export function genericElementPostProcess(
        plugin: Plugin,
        source: string, 
        el: HTMLElement, 
    ...
    // be indented
    let wrapper_class: string = edit_mode_safe ? 'ds-element-wrapper-edit-safe' : 'ds-element-wrapper'
    ...
    try {
        ...
        if (typeof viewInstance.build !== 'function') {
            throw new Error(`View class does not have a build() method.`);
        }
        ...
        elementWrapper.createEl("div", { text: userMessage, cls: "error-message" });
    }
}
