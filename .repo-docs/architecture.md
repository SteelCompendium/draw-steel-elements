...
(`ViewRegistry.noteWrite`, via `host.notePersistIntent()`) BEFORE the `Vault.process` call
that causes it, because Obsidian reruns the code-block processor for the new section
during/just after that call. The new section's processor asks `registry.claim(docId,
